# HST v6.2 Accurate - Giga with KV Cache Management
...
device = torch.device('cuda'); os.makedirs('/content/drive/MyDrive/HST_Training/v6_2_accurate', exist_ok=True)
print("HST v6.2 Accurate - Cached Giga")

class CachedGigaBlock(nn.Module):
...
class HSTv62Accurate(nn.Module):
    def __init__(self, d_model=32, num_layers=4):
        super().__init__()
        self.input = nn.Linear(1, d_model)
        self.blocks = nn.ModuleList([CachedGigaBlock(d_model) for _ in range(num_layers)])
...
model = HSTv62Accurate().to(device); opt = torch.optim.Adam(model.parameters(), 1e-3); crit = nn.MSELoss()
for e in range(10):
    for X, in train_loader: opt.zero_grad(); loss = crit(model(X)[:, :-1].squeeze(-1), X[:, 1:]); loss.backward(); opt.step()
    print(f"Epoch {e+1}: Done")
torch.save(model.state_dict(), '/content/drive/MyDrive/HST_Training/v6_2_accurate/hst_v6_2_accurate.pt')
print("✓ HST v6.2 Accurate saved")
