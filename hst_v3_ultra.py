...
from lattice_extension import TrueMultiLayerLattice
from adaptive_bottom import AdaptiveBottomBlock
...
class AdaptiveBottomTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, spine_indices: torch.Tensor, alpha: float = 0.5) -> Tuple[torch.Tensor, int]:
        ...
        # [NEW FIX: Create a modifiable copy after the Transformer layers]
        x_modified = x.clone()
        
        # 3. Preserve Spine Emphasis Logic from original file (Section 2.4)
        pos = spine_indices[0] 
        if pos < x_modified.size(1):
            # Safe, non-inplace update
            pos_tensor = torch.tensor([pos], device=x_modified.device)
            
            # 1. Calculate the update
            x_pos = x_modified.index_select(1, pos_tensor) # [B, 1, D]
            update = x_pos * (1 + alpha * 0.5)

            # 2. Apply the update non-inplace
            x_modified = x_modified.index_copy(1, pos_tensor, update)

        return x_modified, predicted_depth
...
class PredictionLatticeCore(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # [NEW FIX: Clone the input to prevent modifying a variable needed by upstream BWD pass]
        x_modified = x.clone()
        
        lattice_indices = self.lattice.spine_indices
        B, S, D = x_modified.shape
        ...
        for k, pos_z in enumerate(relevant_indices):
            ...
            h_lattice = x_modified[:, pos_z, :] * 0.5 
            h_attention = x_modified[:, pos_z, :] * 0.5
            ...
            pos_z_tensor = torch.tensor([pos_z], device=x_modified.device)
            x_modified = x_modified.index_copy(1, pos_z_tensor, update.unsqueeze(1)) 
...
if __name__ == '__main__':
    ...
    model = HSTv3Ultra(
        ...
        # === REAL PAPER-COMPLIANT EXTENSIONS ===
        self.multi_lattice = TrueMultiLayerLattice(d_model)
        self.adaptive_bottom = nn.ModuleList([
            AdaptiveBlock(d_model, n_heads) for _ in range(8)
        ])
        self.depth_predictor = nn.Linear(d_model, 1)
        # ======================================

        # === ADAPTIVE BOTTOM TRANSFORMER (real early exit) ===
        confidences = []
        for i, block in enumerate(self.adaptive_bottom):
            ...
            predicted_depth = len(self.adaptive_bottom)
        # =============================================

        # === MULTI-LAYER HIERARCHICAL LATTICE ===
        x = self.multi_lattice(x)
        # ======================================

        closure_loss = self.multi_lattice.closure_loss()
        total_loss = lm_loss + 0.4 * horizon_loss + closure_loss
        output['bottom_depth'] = predicted_depth
