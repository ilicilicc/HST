# HST v6.1 Accurate - Enhanced Giga with Dropout & GELU
...
device = torch.device('cuda'); os.makedirs('/content/drive/MyDrive/HST_Training/v6_1_accurate', exist_ok=True)
print("HST v6.1 Accurate - Enhanced Giga")

class EnhancedGigaBlock(nn.Module):
...
class HSTv61Accurate(nn.Module):
    def __init__(self, d_model=32, num_layers=4):
        super().__init__()
        self.input = nn.Linear(1, d_model)
        self.blocks = nn.ModuleList([EnhancedGigaBlock(d_model) for _ in range(num_layers)])
...
model = HSTv61Accurate().to(device); opt = torch.optim.Adam(model.parameters(), 1e-3); crit = nn.MSELoss()
for e in range(10):
    for X, in train_loader: opt.zero_grad(); loss = crit(model(X)[:, :-1].squeeze(-1), X[:, 1:]); loss.backward(); opt.step()
    print(f"Epoch {e+1}: Done")
torch.save(model.state_dict(), '/content/drive/MyDrive/HST_Training/v6_1_accurate/hst_v6_1_accurate.pt')
print("✓ HST v6.1 Accurate saved")
